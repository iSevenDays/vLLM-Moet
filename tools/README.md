...
| `test_paged_mqa_tf32_flip.py` | 1 GPU | ~1 min | Does the decode indexer's **TF32** score (`allow_tf32=True` at `triton_paged_mqa_logits_dsv4.py:185`) flip borderline top-512 membership? Dials a TARGET token to each fp32 rank 505–520 and compares TF32 vs fp32 top-512 membership + natural signed-weight sampling. The op-level probe of the `dig_decode_path.md` decode-SELECTION suspect (one-boot falsifier: `VLLM_DSV4_PAGED_MQA_LOGITS_FORCE_TORCH=1`). |
| `test_moe_w2_sm89_native_gate.py` | 1 GPU | ~1 min | Full op-gate for the **sm_89-native decode W2 cubin** (the `dig_moe_expert_path.md` suspect whose header TODO was never run). Invokes the existing `kernels/gen/moe_w2_sm89_native_check.py` (static + driver-API gate) AND A/Bs the cubin vs the self-tested Triton emulation on identical descriptors across both decode shapes × seeds × adversarial activations (outlier / all-equal / sign-alternating / max-magnitude); gate = bf16 floor ~2.4e-3. Falsifier: `VLLM_MOE_W2_SM89_NATIVE=0`. |
| `verify_needle_token_position.py` | CPU + model dir | seconds | Where the needle actually is **in tokens**. The probe inserts at a BYTE fraction but reports `int(pt x depth)`, a TOKEN fraction; at ask/8192 that is off by **+970 tokens = 243 ratio-4 columns**, which is what voided the §5.19 rank trace. **Run before aiming any position-addressed instrument.** |
