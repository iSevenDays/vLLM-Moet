...
The active Ada image uses vLLM v0.25.1 at commit `752a3a504`.
The image applies generated per-file patches from `patches/`.
The full source files in `overlay/vllm/` produce those patches.
...
1. **2‑bit experts with FP4 recovery** — routed experts compress to a sign‑symmetric 2‑bit
   ...
   gate) restores precision where it matters.
2. **Tiered expert residency** — when the 2‑bit base outgrows VRAM, it moves to pinned
   host RAM. One tier further, it moves to an **NVMe pack file with a pinned‑RAM arena**.
   The GPU then becomes an **expert cache** (miss → batched fetch + bit‑identical graph replay).
   This puts 753B on two 96 GB cards and 159B on a single RTX 5090. The packs also work as
...
per‑tensor scale_2) to the sign‑symmetric 2‑bit planes at load. The result is f64‑exact vs
the reference pipeline on real shards. Single‑stream, greedy, CUDA graphs:
...
- **4 cards:** prefill ~2.5k tok/s; MTP acceptance 2.3–2.8; needle retrieval **PASS to 126K**
  ...
  without changes. The endpoint drives coding agents (opencode) directly.
- **2 cards — a model that does not fit, running anyway:** the 2‑bit planes alone (~190 GiB)
  ...
  miss‑tolerance 8. Arithmetic and retrieval probes are clean at both settings. Bare‑2‑bit
  quality artifacts ("capital of Poland: Krakow", garbled Polish) are corrected by the FP4
  tier. Booting from existing quantization packs takes **~7 min** (vs ~11 for a full
...
config. These configs use no host expert store.
...
Four consumer 5090s match two PRO 6000s on decode. MTP acceptance is ~2.6 tok/step across
configs. MTP also runs under **pipeline parallelism** (draft propagation + drafter embedding
share across ranks): DS4 on 4× RTX 5090 **PP4** does 184 tok/s vs 93 without (~2×). Greedy
...
- **2‑bit expert planes — the sign‑bias finding.** Naive 2‑bit *destroys* these models
  ...
  codebook drops one sign's tail. The per‑expert bias then compounds over dozens of layers.
...
- **FP4 recovery — used surgically.** Decode is HBM‑bound and an FP4 read is 2× the bytes.
  So 2‑bit is the *fast* default: a **delta cache** keeps the hot experts at FP4 (background
  promote/evict, CUDA‑graph‑safe, `VLLM_MOE_W2_DELTA_GB=auto` sizes it from post‑KV VRAM).
  A **confidence gate** (`VLLM_MOE_W2_GATE=1`) re‑runs low‑confidence steps at FP4. The gate
  force‑promotes the step's routed experts, replays the graph once, and re‑decides. This
  works inline on TP/single‑GPU (incl. MTP verify steps) and as a full‑pipeline replay under
  PP. Set τ at runtime.
- **The kernels.** `moe_w2_mm` (2‑bit MoE GEMM: PRMT‑LUT in‑register decode → `QMMA.SF`
  ...
  shipped as sources + prebuilt cubins for every sharding (K = 6144/4096/2048/1024/512).
  So TP2/TP4 work without changes. Op‑validated (rel ~1–3e‑3, deterministic),
  graph‑capture‑exact. Prefill runs the **AFRAG** variant (fragment‑major activations → one
  `LDG.128` per QMMA A‑fragment; the prefill GEMM is load‑issue‑bound, not DRAM‑bound):
  bit‑identical outputs, 1.3× on the GEMM, **+12% e2e prefill** on one card — default on
  (`VLLM_MOE_W2_AFRAG=0` opts out).

On Ada, the decode tier of the 2-bit MoE GEMM runs a native CUDA cubin: PRMT‑LUT decode to
e4m3 in registers → Ada's native FP8 `mma.m16n8k32`. The kernel is parity‑gated at every
boot, with a Triton fallback on any failure (measured +27% single‑stream decode / +66% at
concurrency 4 e2e). Prefill and all other shapes run the Triton emulation, which decodes the
expert codes to BF16 in registers and uses BF16 tensor cores. DeepSeek-V4 uses a separate
native SM89 FP8 kernel for its attention output projection. This kernel does not materialize
dequantized FP32 weights. See the Ada port guide for the dispatch rules and validation
results.
...
latter two are re‑quantized to the sign‑symmetric codebook at load, float64‑exact vs the
reference pipeline.
...
coverage serves ~96% of token→expert routings** on DS4, **~51% serves ~91%** on GLM. These
numbers are measured live, not simulated.

Misses stay correct through the gate's replay trick: the desc kernel zeroes a missing
expert's contribution and bumps an in‑graph miss counter. The runner then fetches **all**
missing routed experts in one batched pinned‑H2D transfer (51.6 GiB/s here; a 64‑expert
fetch ≈ 3 ms) and replays the step's graph once. The result is **bit‑identical** to a fully
resident forward (unit‑tested). A **miss‑tolerance knob**
(`VLLM_MOE_W2_BASE_MISS_TOL=k`, runtime‑tunable) skips the replay when ≤ k of the step's
~600 routings miss. This gives +12% decode on GLM TP2 at tol 8 (28.3 → 31.7 tok/s) with
clean quality probes (arithmetic, PL coherence, needle retrieval; quantitative eval pending).

**Pool size is the dominant knob — treat it as a config KPI.** The mandatory replay is paid
*per step*, so decode tracks the fraction of **zero‑miss steps**. That fraction falls off a
cliff with coverage, while the token hit‑rate barely moves. On DS4 (1× 5090, NVMe‑store
stack, same box, same bench): 11 GiB pool / util 0.90 = **27–28 tok/s**, 14 GiB / util 0.95
...
e.g. `--gpu-memory-utilization 0.95`) before you touch any other knob.
...
= the mandatory first‑order restore only, the throughput‑optimal setting). Raising the
threshold buys bit‑deterministic fixed points on converged working sets at a decode cost
(runtime‑tunable via `VLLM_MOE_W2_FP_THRESH_FILE`). The accepted residue is second‑order
only and is KPI‑visible (`fp-residue`).

Results: **DeepSeek‑V4‑Flash 159B on one RTX 5090** (72.7 GiB of 2‑bit planes vs 32 GB of
VRAM): ~31 tok/s steady with MTP, 32K window served, coherent. The stack uses ~30 GiB of
host RAM with the NVMe stores (below, RSS‑measured) instead of ~80 GiB pinned.
...
table above. Neither model can run on that hardware at any precision without this stack.
...
sidecar with shapes and the layers written. Three things follow:

- **The RAM wall falls.** Single‑5090 DS4 no longer needs ~80 GiB of free host RAM
  (measured host RAM of the serving process: 42–44 → 26–33 GiB). GLM TP2's expert stores go
  **~568 → ~136 GiB** host RAM. The config then fits hosts that could never hold the pinned
  stores.
- **The pack is a persistent quantization cache.** The first boot writes the pack while it
  quantizes. Every later boot **skips the dequant→re‑quant entirely** and serves experts
...
needle retrieval 4/4 to **121K prompt tokens** at a served 128K window. To enable it, add
two env lines on any base‑cache config (DS4 shown; the same two lines serve GLM TP2):
...
- **Disk budget:** DS4 pack 75 GB; GLM TP2 2×100 GB (base) + 2×189 GB (fp4) — plan ~1 TB of
  ...
  boot, so SSD wear is not an issue. The dir must be a real filesystem via bind mount — **not
  overlayfs** (the container filesystem).
- **You do not need a fast drive for steady decode.** Misses are buffered reads, so the page
...
- **Prefill cannot wipe the arena** (scan discipline: prefill working sets fill free slots
  but never evict the decode hot set). The arena's hot set persists to
...
serve them on SM120. The patch carries the fixes (details in
...
- **flashinfer**: the official 0.6.12 pin predates the SM120 DS4 attention API → **0.6.14**.
...
With the 2‑bit knobs off, the patch is exactly these base fixes. The runtime is otherwise
stock.
...
Image: `vllm-moet-sm89:v0251` (built locally; keep it, tag baselines as suffixed
...
checkpoint from HuggingFace on first run. It starts the exact configuration
...
added to the vLLM tree by the patch). The packed-write kernel is prebuilt at
...
Drop the two `STORE_DIR`/`BASE_RAM_GB` lines for the all‑RAM variant. That variant then needs
...
for the results, the limits, and the required Ada comparison.
...
and is unreachable from CUDA/PTX. So the stack underneath this repo is end‑to‑end ours:
...
- **[`cubit`](https://github.com/kacper-daftcode/cubit)** — an **SM120 SASS assembler/disassembler**
  ...
  cubins this server loads. It is the only tool needed to rebuild or audit them.
...
kernels do not. This toolchain is what makes them possible.
...
The 2‑bit planes total **265.8 GiB** (~66 GiB/rank + ~8 GiB BF16 dense/vision). This leaves
...
round‑trip works. Serve recipe and bring‑up findings (a checkpoint‑specific
**zero‑sign balancing** fix the sweep gate caught — the INT4→NVFP4 export writes all
exact zeros as +0, which would inject 3× the bias that degenerates GLM; an SM12x smem
fix for dense‑MLA triton decode; a `merge_attn_states` stride fix for >64K chunked
prefill):
...
† imported from pre-harness measurements (README/docs history) — to be re-measured on the next release.
...
† imported from the measurement campaign logs — to be re-measured by the harness on the next quality release.

<!-- bench:quality:end -->

## Repository layout

- **`overlay/vllm/`** holds the complete modified vLLM files. Edit these files.
- **`patches/`** holds the generated per-file patches for official vLLM v0.25.1.
...
- **`kernels/`** holds the kernel sources, generated files, cubins, and validation tools.
...
- **`bench/`** holds benchmark recipes, the runner, and committed results.
