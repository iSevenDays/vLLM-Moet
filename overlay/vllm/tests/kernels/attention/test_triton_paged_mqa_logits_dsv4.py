...
def test_pack_roundtrip_cpu():
    ...
    # Segregated per-block layout (see pack_fp8_indexer_cache_reference /
    # indexer_k_quant_and_cache_kernel): each block holds block_size*D k bytes
    # then block_size*4 scale bytes -- NOT [D+4] per token.
    kv = cache.reshape(num_blocks, block_size * (D + 4))
    kq = kv[:, :block_size * D].reshape(-1, D)
    ks = kv[:, block_size * D:].reshape(-1, 4)
    deq = (kq.view(torch.float8_e4m3fn).float()
