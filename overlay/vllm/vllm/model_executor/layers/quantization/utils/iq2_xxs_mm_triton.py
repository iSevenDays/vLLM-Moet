...
#
# Matches antirez's ``dev_dot_iq2_xxs_q8_K_block`` (ds4_cuda.cu:16962) exactly:
# int8 activations (Q8_K-quantized) DP4A int8 weights (sign-applied grid values
# in {-43..43}) -> int32 accumulate -> fp32 fold with 0.125 * d_w * d_a.
#
# This is the path the IQ2_XXS quantization was calibrated against.  The bf16
# variant above produces a slightly different (actually MORE fp32-accurate)
# result that diverges from the calibration target by ~0.8% worst_rel per dot;
# across 43 layers x long context this compounds and degrades needle accuracy.
# ===========================================================================

@triton.jit
def _iq2_xxs_mm_q8k_kernel(
    ...
    for kb in range(BLOCKS_PER_ROW):
        # ---- IQ2_XXS block base + d (fp16 weight scale) ----
        block_base = offs_n * stride_wn + kb * BLOCK_BYTES         # [BLOCK_N]
        d_lo = tl.load(w_ptr + block_base + 0, mask=mask_n, other=0).to(tl.uint32)
        d_hi = tl.load(w_ptr + block_base + 1, mask=mask_n, other=0).to(tl.uint32)
        d_u16 = (d_lo | (d_hi << 8)).to(tl.uint16)
        d_fp16 = d_u16.to(tl.float16, bitcast=True).to(tl.float32)  # [BLOCK_N]

        # ---- Q8_K block scale for this kb (per-M-row fp32) ----
        a_d = tl.load(a_d_ptr + offs_m * stride_adm + kb, mask=mask_m, other=0.0)  # [BLOCK_M]

        # per-kb int32 accumulator (reset each kb to bound the magnitude).
        acc_i32 = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.int32)

        for ib32 in tl.static_range(8):
            sub_base = block_base + 2 + ib32 * 8                   # [BLOCK_N]

            # Load 4 uint16 qs and pack into aux0/aux1 (identical to bf16 kernel).
            q0 = tl.load(w_ptr + sub_base + 0, mask=mask_n, other=0).to(tl.uint32) | \
                 (tl.load(w_ptr + sub_base + 1, mask=mask_n, other=0).to(tl.uint32) << 8)
            q1 = tl.load(w_ptr + sub_base + 2, mask=mask_n, other=0).to(tl.uint32) | \
                 (tl.load(w_ptr + sub_base + 3, mask=mask_n, other=0).to(tl.uint32) << 8)
            q2 = tl.load(w_ptr + sub_base + 4, mask=mask_n, other=0).to(tl.uint32) | \
                 (tl.load(w_ptr + sub_base + 5, mask=mask_n, other=0).to(tl.uint32) << 8)
            q3 = tl.load(w_ptr + sub_base + 6, mask=mask_n, other=0).to(tl.uint32) | \
                 (tl.load(w_ptr + sub_base + 7, mask=mask_n, other=0).to(tl.uint32) << 8)
            aux0 = q0 | (q1 << 16)
            aux1 = q2 | (q3 << 16)

            ls_field = (aux1 >> 28) & 0xf
            ls = (2 * ls_field + 1).to(tl.int32)                  # [BLOCK_N] odd [1,31]

            a_k = (aux0[:, None] >> (8 * k4[None, :])) & 0xff      # [BLOCK_N, 4]
            sign_k = (aux1[:, None] >> (7 * k4[None, :])) & 0x7f   # [BLOCK_N, 4]

            grid_offs = a_k[:, :, None] * 8 + i8[None, None, :]    # [BN, 4, 8]
            grid_vals = tl.load(grid_ptr + grid_offs).to(tl.int32)  # values in {8, 25, 43}

            sign_byte = tl.load(ksigns_ptr + sign_k).to(tl.int32)         # [BN, 4]
            sign_bit = (sign_byte[:, :, None] >> i8[None, None, :]) & 1   # [BN, 4, 8]
            sign_factor = (1 - 2 * sign_bit).to(tl.int32)                 # [BN, 4, 8] in {-1, +1}

            signed = grid_vals * sign_factor                               # [BN, 4, 8] in {-43..43}
            # Flatten to [BLOCK_N, 32] int8 -- values fit (|v| <= 43 < 128).
            w_tile_i8 = tl.reshape(signed, (BLOCK_N, 32)).to(tl.int8)

            # Load int8 activations [BLOCK_M, 32] for this sub-block's K-window.
            k_glob = kb * 256 + ib32 * 32 + i32                  # [32]
            a_tile = tl.load(
            ...
            # DP4A int8 x int8 -> int32.  out_dtype=int32 maps to IMMA on sm_89.
            sumi = tl.dot(a_tile, tl.trans(w_tile_i8), out_dtype=tl.int32)  # [BLOCK_M, BLOCK_N]

            # Scale by per-sub-block ls (int32, exact) and accumulate.
            acc_i32 += sumi * ls[None, :]

        # Per-kb fp32 fold: 0.125 * d_w * d_a * bsum.
...
def iq2_xxs_mm_q8k(
    ...
    *,
    ...
    Computes ``C[M,N] = sum_k (a_qs[m,k] * int8_w[n,k]) * 0.125 * d_w[n,kb] *
    a_d[m,kb]`` where ``int8_w`` is the IQ2_XXS grid value with sign applied
    (values in {-43, -25, -8, 8, 25, 43}) and the int8xint8 dot uses DP4A.

    Args:
        a_qs: int8 ``[M, K]`` Q8_K-quantized activations.
        a_d: fp32 ``[M, K // 256]`` per-block Q8_K scales.
