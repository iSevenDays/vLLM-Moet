...
# FP8-e4m3 delta tier (Ada native FP8 MMA): 1 byte/elem = 2x the FP4-full
# footprint (N*K//2). Slots carry ONLY the fragment-major e4m3 byte plane —
# no scale section (the UE8M0 block-32 scale plane is the SHARED resident
# base scale). The unit-space e4m3 is derived from the same requant pipeline
# as the 2-bit codes, so no SCALE_REFIT conflict (unlike FP4).
W8_13_BYTES = 4096 * 4096             # 16.0 MiB (TP1)
W8_2_BYTES = 4096 * 2048              # 8.0 MiB (TP1)

# FP8-e4m3 delta tier env (default OFF — mutually exclusive with the FP4
# delta tiers). When enabled, serve sets VLLM_MOE_W2_DELTA_GB=0 (FP4 off).
_FP8_DELTA_ON = os.getenv("VLLM_MOE_W2_FP8_DELTA", "0").lower() not in (
    "0", "", "false", "no", "off")
_FP8_GB_RAW = os.getenv("VLLM_MOE_W2_FP8_DELTA_GB", "2.0").strip().lower()
_FP8_GB = 0.0 if _FP8_GB_RAW in ("auto", "-1", "-1.0") else float(_FP8_GB_RAW)


_FP8_STORE = os.getenv("VLLM_MOE_W2_FP8_STORE", "fp8").strip().lower()
if _FP8_STORE not in ("fp8", "fp4"):
    raise ValueError(
...
def fp8_store_fp4() -> bool:
    ...
    hold the checkpoint e2m1 nibble plane (half the bytes of the e4m3 byte
    plane); the kernel decodes nibble->e4m3 in registers. Exact vs fp8-store
    (e2m1 is a subset of e4m3) PROVIDED the serving scale equals the
    checkpoint scale — hence the SCALE_REFIT=0 requirement enforced at
    build (a refit-decremented scale needs u = 2*e2m1, off the e2m1 grid)."""
    return _FP8_DELTA_ON and _FP8_STORE == "fp4"


def fp8_enabled() -> bool:
    """FP8-e4m3 delta PREFILL tier (Ada native FP8 MMA).

    Mutually exclusive with the FP4 delta tiers (``enabled``): when FP8 is
    active, the serve script pins VLLM_MOE_W2_DELTA_GB=0 so the FP4 tier is
    inert. Because the FP8 unit-space e4m3 is derived from the SAME requant
    pipeline as the 2-bit codes (including any scale refit), enabling FP8
    does NOT force ``VLLM_MOE_W2_SCALE_REFIT=0`` (the FP4 tier does, via
    ``scale_refit_enabled``, because FP4 reuses the original checkpoint
...
class DeltaTier:
    def __init__(self, n_layers: int, n_experts: int, dev,
        ...
        # Deferred pool (defer_pool=True — the FP8 prefill tier): the tier is
        # constructed INSIDE build_layer_planes (via _fp8_tier_for_build), where
        # a multi-GiB pool competes with the build's per-layer GPU workspaces on
        # top of the resident base planes (measured: 1 GiB workspace alloc
        # failure at layer ~40 with a 3.5 GiB pool). finalize_auto (worker hook,
        # post-KV, pre-capture) allocates it instead; _explicit_gb caps the
        # auto sizing when the env named a size ("auto"/0 -> pure free-VRAM).
        self._explicit_gb = float(_gb) if (defer_pool and _gb > 0) else None
        self._auto_pending = (_AUTO and pool_gb is None) or defer_pool
    ...
    def finalize_auto(self) -> None:
        ...
        if self._explicit_gb is not None:
            want = int(self._explicit_gb * 2**30)
            if want > budget:
                logger.warning(
                    "moe_w2 [%s] deferred pool: requested %.2f GiB exceeds the "
                    "post-KV budget (%.2f GiB free - %.1f GiB reserve) — "
                    "clipping to the budget", self._tag, self._explicit_gb,
                    free_b / 2**30, _RESERVE_GB)
            budget = min(budget, want)
    ...
    def add_layer_host_sections(self, layer_key: int, parts13, parts2):
        ...
        # Staging-size invariant: total staged bytes per expert must equal
        # slot_bytes. Turns any future tier/format drift (e.g. an FP8 tier
        # sized for FP4 half-sizes — the w8 IMA root cause) into a LOUD boot
        # error at the first layer build instead of a silent overread later.
        # All callers pass 2D [E, section_bytes] tensors (the store's
        # contract — see moe_w2_store.add_layer).
        total = sum(t.shape[1] for t in parts13) + sum(t.shape[1]
                                                       for t in parts2)
        if total != self.slot_bytes:
            raise RuntimeError(
                f"moe_w2 [{self._tag}] staging {total} B/expert into "
                f"{self.slot_bytes} B slots — tier sized for a different "
                f"plane format")
    ...
    def _tick_once(self):
        if self._tag == "w8" and not self._free:
            # w8: frozen once full FOR THE BACKGROUND MANAGER only. A long
            # prefill's working set (~256 experts/layer) >> pool, so background
            # adaptivity just churns slots and races the desc kernel's pool
            # reads. Coverage comes from the SYNCHRONOUS per-chunk
            # ensure_resident (forward thread, emergency=True), which bypasses
            # this freeze in _take_slots_batch — the sm120-proven dynamic path
            # without the background-race surface.
            return
    ...
    def _take_slots_batch(self, k: int, emergency: bool = False,
        ...
        if self._tag == "w8" and not self._free and not emergency:
            # w8 freeze applies to the background manager only: the synchronous
            # runner/forward-thread callers (force_promote, ensure_resident)
            # pass emergency=True and may evict-and-pin — per-chunk prefill
            # coverage REQUIRES rotating the whole pool every layer.
            return []
        out: list[int] = []
...
def get_tier(n_layers=None, n_experts=256, dev=None,
             w13_bytes=None, w2_bytes=None,
             fp8: bool = False) -> DeltaTier | None:
    global _TIER
    is_fp8 = fp8_enabled()
    # Singleton-poisoning guard (root cause of the w8 IMA at plane-build):
    # _fp4_tier_for_build runs BEFORE _fp8_tier_for_build and calls this fn
    # passing FP4 half-sizes (w13_bytes=n13k13//2). When FP8 is enabled, the
    # plain `enabled()` gate is False but is_fp8 is True, so without this
    # guard the FP4 call site would construct/receive the singleton FP8 tier
    # sized for the WRONG (FP4) plane format -> slots 6 MiB instead of 12 MiB
    # -> the w8 GEMM overreads each slot -> illegal memory access at the
    # tick's sync. Only the FP8 call site (fp8=True) may build/receive the
    # FP8 tier; FP4/split call sites are inert in FP8 mode.
    if is_fp8 and not fp8:
        return None
    if not enabled() and not is_fp8:
        return None
    if _TIER is None:
        if n_layers is None:
        ...
        if is_fp8:
            # FP8-e4m3 delta tier (Ada native FP8 MMA). Slots carry only the
            # e4m3 byte plane (w13_bytes = N13*K13, w2_bytes = N2*K2, passed
            # by the plane builder); the UE8M0 scale plane is the shared
